...
public class SqlConstraintUtils {
        ...
        boolean restrictMemberTypes)
    {
        ...
        Map<RolapLevel, List<RolapMember>> rolesConstraints =
            getRolesConstraints(evaluator);
        for (RolapLevel l : rolesConstraints.keySet()) {
          List<RolapMember> slicerMembers = rolesConstraints.get(l);
          if (slicerMembers.size() > 0) {
            final String where =
                generateSingleValueInExpr(
                    sqlQuery, baseCube,
            ...
            if (!where.equals("")) {
              ...
              // list of members is greater than the limit
              // permitted, we won't constraint.
              whereClausesForRoleConstraints.put((RolapCubeLevel) l, where);
            }
          }
        }
