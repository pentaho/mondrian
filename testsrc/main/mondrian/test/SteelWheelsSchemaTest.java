...
public class SteelWheelsSchemaTest extends SteelWheelsTestCase {
    ...
    public void testNativeVirtualRestrictedSet() throws Exception {
        ...
        final TestContext ctx = getTestContext().withSchema(
            ...
            + "    </Measure>\n"
            + "  </Cube>\n"
            + "  <VirtualCube enabled=\"true\" name=\"Quadrant Analysis Virtual\" visible=\"true\">\n"
            + "    <CubeUsages>\n"
            + "      <CubeUsage cubeName=\"Quadrant Analysis\" ignoreUnrelatedDimensions=\"false\">\n"
            + "      </CubeUsage>\n"
            + "    </CubeUsages>\n"
            + "    <VirtualCubeDimension visible=\"true\" highCardinality=\"false\" name=\"Region\">\n"
            + "    </VirtualCubeDimension>\n"
            + "    <VirtualCubeDimension visible=\"true\" highCardinality=\"false\" name=\"Department\">\n"
            + "    </VirtualCubeDimension>\n"
            + "    <VirtualCubeDimension visible=\"true\" highCardinality=\"false\" name=\"Positions\">\n"
            + "    </VirtualCubeDimension>\n"
            + "    <VirtualCubeMeasure cubeName=\"Quadrant Analysis\" name=\"[Measures].[Actual]\">\n"
            + "    </VirtualCubeMeasure>\n"
            + "    <VirtualCubeMeasure cubeName=\"Quadrant Analysis\" name=\"[Measures].[Budget]\">\n"
            + "    </VirtualCubeMeasure>\n"
            + "    <VirtualCubeMeasure cubeName=\"Quadrant Analysis\" name=\"[Measures].[Variance]\">\n"
            + "    </VirtualCubeMeasure>\n"
            + "  </VirtualCube>\n"
            + "  <Role name=\"F-MIS-BE-CLIENT\">\n"
            + "    <SchemaGrant access=\"none\">\n"
            + "      <CubeGrant cube=\"Quadrant Analysis Virtual\" access=\"all\">\n"
            + "        <HierarchyGrant hierarchy=\"[Region]\" rollupPolicy=\"partial\" access=\"custom\">\n"
            + "          <MemberGrant member=\"[Region].[All Regions]\" access=\"none\">\n"
            + "          </MemberGrant>\n"
        ...
        ctx.executeQuery(
            "With\n"
            + "Set [*NATIVE_CJ_SET] as 'NonEmptyCrossJoin([*BASE_MEMBERS_Region],[*BASE_MEMBERS_Positions])'\n"
            + "Set [*SORTED_ROW_AXIS] as 'Order([*CJ_ROW_AXIS],[Region].CurrentMember.OrderKey,BASC,[Positions].CurrentMember.OrderKey,BASC)'\n"
            + "Set [*BASE_MEMBERS_Positions] as '[Positions].[Positions].Members'\n"
            + "Set [*BASE_MEMBERS_Region] as '[Region].[Region].Members'\n"
            + "Set [*BASE_MEMBERS_Measures] as '{[Measures].[*FORMATTED_MEASURE_0]}'\n"
            + "Set [*CJ_ROW_AXIS] as 'Generate([*NATIVE_CJ_SET], {([Region].currentMember,[Positions].currentMember)})'\n"
            + "Set [*CJ_COL_AXIS] as '[*NATIVE_CJ_SET]'\n"
            + "Member [Measures].[*FORMATTED_MEASURE_0] as '[Measures].[Budget]', FORMAT_STRING = '#,###.00', SOLVE_ORDER=400\n"
            + "Select\n"
            + "[*BASE_MEMBERS_Measures] on columns,\n"
            + "Non Empty [*SORTED_ROW_AXIS] on rows\n"
            + "From [Quadrant Analysis Virtual]\n");
    }
}
