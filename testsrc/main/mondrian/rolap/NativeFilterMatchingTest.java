...
public class NativeFilterMatchingTest extends BatchTestCase {
    ...
    public void testNativeFilterWithCompoundSlicer_1() {
        ...
        final String sqlMysql =
            ...
            + "    `sales_fact_1997`.`product_id` = `product`.`product_id`\n"
            + "and\n"
            + "    ((`product_class`.`product_subcategory`, `product_class`.`product_category`, `product_class`.`product_department`, `product_class`.`product_family`) in (('Beer', 'Beer and Wine', 'Alcoholic Beverages', 'Drink'), ('Muffins', 'Bread', 'Baked Goods', 'Food')))\n"
            + "and\n"
            + "    `sales_fact_1997`.`time_id` = `time_by_day`.`time_id`\n"
            + "and\n"
