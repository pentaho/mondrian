...
public class SqlQueryTest extends BatchTestCase {
    private boolean _intialValueGenerateFormattedSql;


    protected void setUp() throws Exception {
        _intialValueGenerateFormattedSql =
    ...
    protected void tearDown() throws Exception {
        MondrianProperties.instance().GenerateFormattedSql.
                set(_intialValueGenerateFormattedSql);
    }
    ...
    public void testPredicatesAreOptimizedWhenPropertyIsTrue() {
        assertSqlEqualsOptimzePredicates(
                true,
    ...
    public void testPredicatesAreOptimizedWhenPropertyIsFalse() {
        assertSqlEqualsOptimzePredicates(
                false,
                "select {[Time].[1997].[Q1],[Time].[1997].[Q2]," +
                        "[Time].[1997].[Q3]} on 0 from sales",
    ...
    public void testPredicatesAreOptimizedWhenAllTheMembersAreIncluded() {
        ...
        String expectedSql = "select `time_by_day`.`the_year` as `c0`, " +
                "`time_by_day`.`quarter` as `c1`, " +
                ...
                " `time_by_day`.`quarter`";
        assertSqlEqualsOptimzePredicates(true, mdx, expectedSql);
        assertSqlEqualsOptimzePredicates(false, mdx, expectedSql);
    }

    private void assertSqlEqualsOptimzePredicates(
            boolean optimizePredicatesValue,
            String inputMdx, String expectedSql
    ) {
        final SqlQuery.Dialect dialect = getTestContext().getDialect();
        if (SqlPattern.Dialect.get(dialect) != SqlPattern.Dialect.ACCESS) {
            System.out.println("This test is currently not avaliable for " +
    ...
    public void testDoubleInList() {
        ...
        patterns =
                new SqlPattern[]{
                        new SqlPattern(SqlPattern.Dialect.LUCIDDB,
        ...
        MondrianProperties.instance().IgnoreInvalidMembers
                .set(origIgnoreInvalidMembers);
        MondrianProperties.instance().IgnoreInvalidMembersDuringQuery
                .set(origIgnoreInvalidMembersDuringQuery);
    }
}
