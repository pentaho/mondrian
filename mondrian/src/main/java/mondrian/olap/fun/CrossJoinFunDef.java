...
public class CrossJoinFunDef extends FunDefBase {
  ...
  private static class MeasureVisitor extends MdxVisitorImpl {
     ...
     * @param crossJoinCall   Measures referencing this call should be excluded from the list of measures found
     */
    MeasureVisitor(
   ...
   * one wants to consider all data across the scope of these other Members. For instance, if Time is not a List-Member,
   * then one wants to consider data across All Time. Or, if Customer is not a List-Member, then look at data across All
   * Customers. The theory here, is if there is no data for a particular Member of the list where all other Members not
   * part of the list are span their complete hierarchy, then there is certainly no data for Members of that Hierarchy
   * at a more specific Level (more on this below).
   *
   * <p>When a Member that is a non-List-Member is part of a Hierarchy
   * that has an All Member (hasAll="true"), then its very easy to make sure that the All Member is used during the
   * optimization. If a non-List-Member is part of a Hierarchy that does not have an All Member, then one must, in fact,
   * iterate over all top-level Members of the Hierarchy!!! - otherwise a List-Member might be excluded because the
   * optimization code was not looking everywhere.
   *
   * <p>Concerning default Members for those Hierarchies for the
   * non-List-Members, ignore them. What is wanted is either the All Member or one must iterate across all top-level
   * Members, what happens to be the default Member of the Hierarchy is of no relevant.
   *
   * <p>The Measures Hierarchy has special considerations. First, there is
   * no All Measure. But, certainly one need only involve Measures that are actually in the query... yes and no. For
   * Calculated Measures one must also get all of the non-Calculated Measures that make up each Calculated Measure.
   * Thus, one ends up iterating across all Calculated and non-Calculated Measures that are explicitly mentioned in the
   * query as well as all Calculated and non-Calculated Measures that are used to define the Calculated Measures in the
   * query. Why all of these? because this represents the total scope of possible Measures that might yield a non-null
   * value for the List-Members and that is what we what to find. It might be a super set, but thats ok; we just do not
   * want to miss anything.
   *
   * <p>For other Members, the default Member is used, but for Measures one
   * should look for that data for all Measures associated with the query, not just one Measure. For a dense dataset
   * this may not be a problem or even apparent, but for a sparse dataset, the first Measure may, in fact, have not data
   * but other Measures associated with the query might. Hence, the solution here is to identify all Measures associated
   * with the query and then for each Member of the list, determine if there is any data iterating across all Measures
   ...
   * element of the input parameter List is only not included in the returned result List if for no combination of
   * Measures, non-All Members (for Hierarchies that have no All Members) and evaluator default Members did the element
   * evaluate to non-null.
   *
   * @param evaluator Evaluator
   * @param list      List of members or tuples
   * @param call      Calling ResolvedFunCall used to determine what Measures to use
   ...
   *
   * @param nonAllMembers array of Member arrays of top-level Members for Hierarchies that have no All Member.
