...
public class XmlaHandler {
   ...
   * "password", but we do expect any olap4j driver that supports i18n to use this property name.
   */
  ...
  public static XmlaExtra getExtra( OlapConnection connection ) {
    try {
      ...
      // Connection cannot provide an XmlaExtra. Fall back and give a default implementation.
    } catch ( UndeclaredThrowableException ute ) {
      // Note: this is necessary because we use a dynamic proxy for the connection.
      // I could not catch and un-wrap the Undeclared Throwable within the proxy.
      // The exception comes out here and I couldn't find any better ways to deal with it.
      // The undeclared throwable contains an Invocation Target Exception which in turns contains the real exception
      // thrown by the "unwrap" method, for example OlapException.
      ...
      // this maintains the original behaviour: don't catch exceptions that are not subclasses of SQLException
    ...
    { // <AxesInfo>
      ...
      { // <AxisInfo>
        ...
        { // <HierarchyInfo>
          ...
          writer.element( "xsd:element", "name", "DisplayInfo", "type", "PropType", "minOccurs", 0, "maxOccurs",
            "unbounded" );
          writer.endElement(); // xsd:sequence
  ...
  private static String sqlToXsdType( final int sqlType, final int scale ) {
    switch ( sqlType ) {
      ...
      case Types.NUMERIC:
      case Types.DECIMAL:
        // Oracle reports all numbers as NUMERIC. We check the scale of the column and pick the right XSD type.
        if ( scale == 0 ) {
          return XSD_INT;
        } else {
          return XSD_DECIMAL;
  ...
  private static Enumeration.ResponseMimeType getResponseMimeType( XmlaRequest request ) {
    Enumeration.ResponseMimeType mimeType =
      Enumeration.ResponseMimeType.MAP.get( request.getProperties().get( PropertyDefinition.ResponseMimeType.name() ) );
  ...
  private static String createCsv( Iterable<?> iterable ) {
  ...
  public OlapConnection getConnection( XmlaRequest request, Map<String, String> propMap ) throws OlapException {
    ...
    if ( sessionId == null ) {
      // With a Simba O2X Client session ID is only null when serving "discover datasources". Let's have a magic ID
      // for the non-authenticated session.
      //
      // REVIEW: Security hole?
      sessionId = "<no_session>";
    }

    LOGGER.debug( "Creating new connection for user [{}] and session [{}]", request.getUsername(), sessionId );
  ...
  private void checkFormat( XmlaRequest request ) throws XmlaException {
    ...
    if ( request.isDrillThrough() ) {
      ...
      if ( format != Format.Tabular ) {
        throw new XmlaException( CLIENT_FAULT_FC, HSB_DRILL_THROUGH_FORMAT_CODE, HSB_DRILL_THROUGH_FORMAT_FAULT_FS,
  ...
  private void execute( XmlaRequest request, XmlaResponse response ) throws XmlaException {
    ...
    Content content = Util.lookup( Content.class, contentName,
      responseMimeType == Enumeration.ResponseMimeType.JSON ? Content.Data : Content.DEFAULT );
    // Handle execute
    ...
    try {
      ...
      boolean rowset = request.isDrillThrough() || Format.Tabular.name()
        .equals( request.getProperties().get( PropertyDefinition.Format.name() ) );
      writer.startElement( "root", "xmlns",
        result == null ? NS_XMLA_EMPTY : ( rowset ? NS_XMLA_ROWSET : NS_XMLA_MDDATASET ),
        "xmlns:xsi", NS_XSI, "xmlns:xsd", NS_XSD, "xmlns:EX", NS_XMLA_EX );
      ...
      try {
        ...
        throw new XmlaException( SERVER_FAULT_FC, HSB_EXECUTE_UNPARSE_CODE, HSB_EXECUTE_UNPARSE_FAULT_FS, e );
      } finally {
  ...
  private QueryResult executeDrillThroughQuery( XmlaRequest request ) throws XmlaException {
    ...
    try ( OlapConnection connection = getConnection( request, Collections.emptyMap() );
          OlapStatement statement = connection.createStatement();
          ResultSet resultSet = getExtra( connection )
            .executeDrillthrough( statement, request.getStatement(), advanced, tabFields, rowCountSlot ) ) {
    ...
    try {
      ...
      try {
        ...
        if ( format == Format.Multidimensional ) {
          dataSet = new MDDataSet_Multidimensional( cellSet, content != Content.DataIncludeDefaultSlicer,
  ...
  private void discover( XmlaRequest request, XmlaResponse response ) throws XmlaException {
    ...
    if ( Objects.requireNonNull( content ) == Content.Schema || content == Content.SchemaData ) {
      rowset.rowsetDefinition.writeRowsetXmlSchema( writer );
    }

    try {
      if ( content == Content.Data || content == Content.SchemaData ) {
   ...
   *
   * <p>If you want to pass in a role object, and you are making the call within the same JVM (i.e. not RPC),
   * register the role using {@link mondrian.olap.MondrianServer#getLockBox()} and pass in the moniker for the
   * generated lock box entry. The server will retrieve the role from the moniker.
   *
  ...
  protected OlapConnection getConnection( String catalog, String schema, final String role, Properties props )
    ...
    try {
      ...
      throw new XmlaException( CLIENT_FAULT_FC, HSB_CONNECTION_DATA_SOURCE_CODE, HSB_CONNECTION_DATA_SOURCE_FAULT_FS,
        e );
    }
  }
  ...
  /**
   * Extra support for XMLA server. If a connection provides this interface, the XMLA server will call methods in
   * this interface instead of relying on the core olap4j interface.
   *
   * <p>The {@link mondrian.xmla.XmlaHandler.XmlaExtraImpl} class provides a default implementation that uses the
   * olap4j interface exclusively.
   */
  public interface XmlaExtra {
    ...
    /**
     * Returns a list of the data sources in this server. One element per data source, each element a map whose keys
     * are the XMLA fields describing a data source: "DataSourceName", "DataSourceDescription", "URL", etc.
    ...
    /**
     * Returns the number of rows returned by a drillthrough on the specified cell. Will also return -1 if it cannot
   ...
   *
   * <p>A typical implementation will probably just use a {@link javax.sql.DataSource} or a connect string, but it is
   * important that the connection is assigned to the correct catalog, schema and role consistent with the client's
   * XMLA context.
   */
  public interface ConnectionFactory {
    ...
    /**
     * Returns a map of property name-value pairs with which to populate the response to the DISCOVER_DATASOURCES
     * request.
     *
     * <p>Properties correspond to the columns of that request: ""DataSourceName", et cetera.</p>
     *
     * <p>Returns null if there is no pre-configured response; in which case, the driver will have to connect to get
     * a response.</p>
     *
  ...
  /**
   * Takes a DataType String (null, Integer, Numeric or non-null) and Value Object (Integer, Double, String, other)
   * and canonicalizes them to XSD data type and corresponding object.
   * <p>
   * If the input DataType is Integer, then it attempts to return an XSD_INT with value java.lang.Integer (and
   * failing that an XSD_LONG (java.lang.Long) or XSD_INTEGER (java.math.BigInteger)). Worst case is the value loses
   * precision with any integral representation and must be returned as a decimal type (Double or java.math.BigDecimal).
   * <p>
   * If the input DataType is Decimal, then it attempts to return an XSD_DOUBLE with value java.lang.Double (and
   * failing that an XSD_DECIMAL (java.math.BigDecimal)).
   */
  @SuppressWarnings( { "UnnecessaryBoxing", "java:S2129" } )
  static class ValueInfo {
    ...
    ValueInfo( final String dataType, final Object inputValue ) {
      ...
      // This is a hint: should it be a string, integer or decimal type. In the following, if the hint is integer,
      // then there is an attempt that the value types be XSD_INT, XST_LONG, or XSD_INTEGER (but they could turn out
      // to be XSD_DOUBLE or XSD_DECIMAL if precision is loss with the integral formats). It the hint is a decimal
      // type (double, float, decimal), then a XSD_DOUBLE or XSD_DECIMAL is returned.
      if ( valueTypeHint != null ) {
        // The value type is a hint. If the value can be converted to the data type without precision loss, ok;
        ...
        if ( valueTypeHint.equals( XSD_STRING ) ) {
          ...
          // If valueTypeHint is XSD_INT, then see if value can be converted to (first choice) integer, (second
          // choice), long and (last choice) BigInteger - otherwise must use double/decimal.
          // Most of the time value ought to be an Integer so try it first
          if ( inputValue instanceof Integer ) {
            // For integer, it's already the right type
            this.valueType = valueTypeHint;
            this.value = inputValue;
            this.isDecimal = false;
            ...
            if ( bi.equals( BigInteger.valueOf( lval ) ) ) {
              // It can be converted from BigInteger to long without loss of precision.
              setValueAndType( lval );
            } else {
            ...
            if ( f.equals( new Float( lval ) ) ) {
              // It can be converted from double to long without loss of precision.
              setValueAndType( lval );
            } else {
              // It can not be converted to a long.
            ...
            try {
              // Can it be converted to a long Throws ArithmeticException on conversion failure. The following line
              // is only available in Java5 and above:
            ...
            // Don't know what Number type we have here. Note: this could result in precision loss.
            this.value = ( (Number) inputValue ).longValue();
            this.valueType = valueTypeHint;
            this.isDecimal = false;
          } else {
            // Who knows what we are dealing with, hope for the best?!?
            this.valueType = valueTypeHint;
            this.value = inputValue;
            this.isDecimal = false;
          }
        } else if ( valueTypeHint.equals( XSD_DOUBLE ) ) {
          // The desired type is double. Most of the time value ought to be a Double so try it first
          if ( inputValue instanceof Double ) {
            // For Double, it's already the right type
            this.valueType = valueTypeHint;
            this.value = inputValue;
            this.isDecimal = true;
            ...
            // What should be done here? Convert ot BigDecimal and see if it can be a double or not? See if there is
            // loss of precision in the conversion? Don't know. For now, just keep it an integral value.
            BigInteger bi = (BigInteger) inputValue;
            // See if it can be an integer or long
            long lval = bi.longValue();
        ...
        if ( inputValue instanceof String ) {
          ...
          try {
            BigDecimal bd2 = Util.makeBigDecimalFromDouble( dval );
            // Can it be a double Must use compareTo - see BigDecimal.equals
            if ( bd.compareTo( bd2 ) == 0 ) {
              this.valueType = XSD_DOUBLE;
              this.value = dval;
            } else {
    ...
    private void setValueAndType( long lval ) {
      if ( !isValidXsdInt( lval ) ) {
        // No, it can not be an integer, must be a long
        this.valueType = XSD_LONG;
        this.value = lval;
      } else {
        // It's an integer.
  ...
  static class TabularRowSet implements QueryResult {
     ...
     *
     * <p>Does not close the ResultSet, on success or failure. Client must do it.
     *
     * @param rs         Result set
     * @param totalCount Total number of rows. If >= 0, writes the "totalCount" attribute into the XMLA response.
     * @throws SQLException on error
     */
    public TabularRowSet( ResultSet rs, int totalCount ) throws SQLException {
      ...
      // Populate data; assume that SqlStatement is already positioned on first row (or isDone() is true), and assume
      // that the number of rows returned is limited.
      rows = new ArrayList<>();
     ...
     *
     * @param tableFieldMap Map from table name to a list of the names of the fields in the table
    ...
    public void unparse( SaxWriter writer ) throws SAXException {
      ...
      for ( Object[] row : rows ) {
        ...
        for ( int i = 0; i < row.length; i++ ) {
          writer.startElement( columns.get( i ).encodedName, "xsi:type", columns.get( i ).xsdType );
          Object value = row[ i ];
    ...
    public void metadata( SaxWriter writer ) {
      ...
      { // xsd:complexType name="row"
        ...
        for ( Column column : columns ) {
          writer.element( "xsd:element", "minOccurs", 0, "name", column.encodedName, "sql:field", column.name, "type",
            column.xsdType );
        }
  ...
  abstract static class MDDataSet implements QueryResult {
  ...
  static class MDDataSet_Multidimensional extends MDDataSet {
    ...
    protected MDDataSet_Multidimensional( CellSet cellSet, boolean omitDefaultSlicerInfo, boolean json )
      throws SQLException {
      super( cellSet );
    ...
    private void olapInfo( SaxWriter writer ) throws OlapException {
      // What are all the cube's hierarchies
    ...
    private boolean isValidProp( List<Position> positions, Property prop ) {
      ...
      for ( Position pos : positions ) {
        if ( pos.getMembers().stream().anyMatch( member -> Objects.nonNull( getHierarchyProperty( member, prop ) ) ) ) {
          return true;
        }
      }
    ...
    private void writeElement( SaxWriter writer, Hierarchy hierarchy, final Property prop ) {
    ...
    private void axes( SaxWriter writer ) throws OlapException {
      ...
      if ( omitDefaultSlicerInfo ) {
        ...
        assert slicerPositions != null;
        final List<Member> slicerMembers =
          slicerPositions.isEmpty() ? Collections.emptyList() : slicerPositions.get( 0 ).getMembers();

        for ( Hierarchy hierarchy : hierarchies ) {
          // Find which member is on the slicer. If it's not explicitly there, use the default member.
    ...
    private void writeMember( SaxWriter writer, Member member, Position prevPosition, Position nextPosition, int k,
      ...
      for ( final Property prop : props ) {
        Object value;
        Property longProp = ( longProps.get( prop.getName() ) != null ) ? longProps.get( prop.getName() ) : prop;
        ...
        if ( longProp == StandardMemberProperty.DISPLAY_INFO ) {
          ...
          value = calculateDisplayInfo( prevPosition, nextPosition, member, k, childrenCard );
    ...
    private Object getHierarchyProperty( Member member, Property longProp ) {
      ...
      if ( thisHierarchyName.equals( thatHierarchyName ) ) {
        try {
          ...
          throw new XmlaException( SERVER_FAULT_FC, HSB_BAD_PROPERTIES_LIST_CODE, HSB_BAD_PROPERTIES_LIST_FAULT_FS, e );
        }
      }
    ...
    private void slicerAxis( SaxWriter writer, Member member, List<Property> props ) throws OlapException {
      ...
      for ( Property prop : props ) {
        ...
        if ( longProp == StandardMemberProperty.DISPLAY_INFO ) {
          Integer childrenCard = (Integer) member.getPropertyValue( StandardMemberProperty.CHILDREN_CARDINALITY );
          // NOTE: don't know if this is correct for SlicerAxis
          value = 0xffff & childrenCard;
        } else if ( longProp == StandardMemberProperty.DEPTH ) {
          value = member.getDepth();
        } else {
    ...
    private int calculateDisplayInfo( Position prevPosition, Position nextPosition, Member currentMember,
                                      int memberOrdinal, int childrenCount ) {
  ...
  /**
   * Callback to handle one column, representing the combination of a level and a property (e.g. [Store].[Store
   * State].[MEMBER_UNIQUE_NAME]) in a flattened dataset.
   */
  static class MemberColumnHandler extends ColumnHandler {
    ...
    public void metadata( SaxWriter writer ) {
      writer.element( "xsd:element", "minOccurs", 0, "name", encodedName, "sql:field", name, "type", XSD_STRING );
    }
  ...
  static class MDDataSet_Tabular extends MDDataSet {
    private static final List<Property> MemberCaptionIdArray =
      Collections.singletonList( StandardMemberProperty.MEMBER_CAPTION );
    ...
    public MDDataSet_Tabular( CellSet cellSet ) {
      ...
      // Count dimensions, and deduce list of levels which appear on non-COLUMNS axes.
      ...
      if ( !localEmpty ) {
        for ( int i = axes.size() - 1; i > 0; i-- ) {
          ...
          // Now we know the lowest levels on this axis, add properties.
      ...
      this.columnHandlers = columnHandlerList.toArray( new ColumnHandler[ 0 ] );
    }
    ...
    private void cellData( SaxWriter writer ) throws OlapException {
      cellOrdinal = 0;
      iterate( writer );
    }
    ...
    private void iterate( SaxWriter writer, int axis, final int xxx ) throws OlapException {
      ...
      for ( int i = 0; i < axisLength; i++ ) {
        ...
        int ho = xxx;
        final List<Member> localMembers = position.getMembers();

        for ( int j = 0; j < localMembers.size() && ho < this.members.length; j++, ho++ ) {
          this.members[ ho ] = position.getMembers().get( j );
        }

        ++cellOrdinal;
        XOMUtil.discard( cellOrdinal );
  ...
  private static class XmlaExtraImpl implements XmlaExtra {
    public ResultSet executeDrillthrough( OlapStatement olapStatement, String mdx, boolean advanced, String tabFields,
                                          int[] rowCountSlot ) throws SQLException {
    ...
    public List<Map<String, Object>> getDataSources(
      ...
      return Collections.singletonList( Olap4jUtil.mapOf(
        "DataSourceName", olapDb.getName(),
    ...
    public Object getOrderKey( Member m ) {
      return m.getOrdinal();
    }
  }
}
