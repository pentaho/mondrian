...
public class OracleDialect extends JdbcDialectImpl {
  ...
  public static final JdbcDialectFactory FACTORY = new JdbcDialectFactory( OracleDialect.class, DatabaseProduct.ORACLE );
   ...
   * <p>
   * The OracleDialect implementation handles some of the specific quirks of Oracle:  e.g. scale = -127 has special
   * meaning with NUMERIC types and may indicate a FLOAT value if precision is non-zero.
   *
   * @param metaData    Resultset metadata
   * @param columnIndex index of the column in the result set
   * @return For Types.NUMERIC and Types.DECIMAL, getType() will return a Type.INT, Type.DOUBLE, or Type.OBJECT
   * based on
  ...
  public SqlStatement.Type getType( ResultSetMetaData metaData, int columnIndex ) throws SQLException {
    ...
    if ( columnType == Types.NUMERIC || columnType == Types.DECIMAL ) {
      ...
      } else if ( columnType == Types.NUMERIC && ( scale == 0 || scale == -127 ) && precision == 0 && columnName.startsWith( "m" ) ) {
        // In GROUPING SETS queries, Oracle loosens the type of columns compared to mere GROUP BY queries.
        // We need integer GROUP BY columns to remain integers, otherwise the segments won't be found;
        // but if we convert measure (whose column names are like "m0", "m1") to integers, data loss will occur.
