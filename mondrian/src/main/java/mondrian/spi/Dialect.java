...
public interface Dialect {
   ...
   * {@code "case when b then 1 else 0 end"} on Oracle, {@code "Iif(b, 1, 0)"} on Access.
   *
  ...
  /**
   * Encloses an identifier in quotation marks appropriate for the current SQL dialect. For example, in Oracle, where
   * the identifiers are quoted using double-quotes,
  ...
  /**
   * Returns whether the SQL dialect allows "AS" in the FROM clause. If so, "SELECT * FROM t AS alias" is a valid
   * query.
   *
   ...
   *
   * @return whether this Dialect supports more than one distinct aggregation in the same query
   */
   ...
   *
   * @return whether this dialect supports multiple count(distinct subquery) measures in one query.
   */
  ...
  /**
   * If Double values need to include additional exponent in its string represenation. This is to make sure that
   * Double
   * literals will be interpreted as doubles by LucidDB.
   *
  ...
  /**
   * Returns whether this dialect supports common SQL Data Definition Language (DDL) statements such as <code>CREATE
   * TABLE</code> and
  ...
  /**
   * Generates an item for an ORDER BY clause, sorting in the required direction, and ensuring that NULL values
   * collate
   * either before or after all non-NULL values, depending on the <code>collateNullsLast</code> parameter.
   *
   * @param expr             Expression
   * @param nullable         Whether expression may have NULL values
   * @param ascending        Whether to sort expression ascending
   * @param collateNullsLast Whether the null values should be sorted first or last.
  ...
  /**
   * Returns whether this Dialect allows the GROUPING SETS construct in the GROUP BY clause. Currently Greenplum, IBM
   * DB2, Oracle, and Teradata.
   *
  ...
  /**
   * Returns whether this Dialect places no limit on the number of rows which can appear as elements of an IN or
   * VALUES
   * expression.
   *
  ...
  /**
   * Returns true if this Dialect can include expressions in the GROUP BY clause only by adding an expression to the
   * SELECT clause and using its alias.
   *
   ...
   *
   * @return Whether this Dialect can include expressions in the GROUP BY clause only by adding an expression to the
   * SELECT clause and using its alias
   */
  boolean requiresGroupByAlias();

  /**
   * Returns true if this Dialect can include expressions in the ORDER BY clause only by adding an expression to the
   * SELECT clause and using its alias.
   *
   * <p>For example, in such a dialect,
   * <blockquote>
   ...
   *
   * @return Whether this Dialect can include expressions in the ORDER BY clause only by adding an expression to the
   * SELECT clause and using its alias
   */
  boolean requiresOrderByAlias();

  /**
   * Returns true if this Dialect can include expressions in the HAVING clause only by adding an expression to the
   * SELECT clause and using its alias.
   *
   * <p>For example, in such a dialect,
   * <blockquote>
   ...
   *
   * @return Whether this Dialect can include expressions in the HAVING clause only by adding an expression to the
   * SELECT clause and using its alias
   */
   ...
   *
   * @return Whether aliases defined in the SELECT clause can be used as expressions in the ORDER BY clause.
   */
  boolean allowsOrderByAlias();

  /**
   * Returns true if this dialect allows only integers in the ORDER BY clause of a UNION (or other set operation)
   * query.
   *
   * <p>For example,
   *
   * <code>SELECT x, y + z FROM t<br/>
   * UNION ALL<br/> SELECT x, y + z FROM t<br/> ORDER BY 1, 2</code>
   * <p>
   * is allowed but
   *
   * <code>SELECT x, y, z FROM t<br/>
   * UNION ALL<br/> SELECT x, y, z FROM t<br/> ORDER BY x</code>
   * <p>
   * is not.
   *
   * <p>Teradata is an example of a dialect with this restriction.
   *
   * @return whether this dialect allows only integers in the ORDER BY clause of a UNION (or other set operation)
   * query
   */
  boolean requiresUnionOrderByOrdinal();

  /**
   * Returns true if this dialect allows an expression in the ORDER BY clause of a UNION (or other set operation) query
   * only if it occurs in the SELECT clause.
   *
   * <p>For example,
   *
   * <code>SELECT x, y + z FROM t<br/>
   * UNION ALL<br/> SELECT x, y + z FROM t<br/> ORDER BY y + z</code>
   * <p>
   * is allowed but
   *
   * <code>SELECT x, y, z FROM t<br/>
   ...
   *
   * @return whether this dialect allows an expression in the ORDER BY clause of a UNION (or other set operation)
   * query
   * only if it occurs in the SELECT clause
   */
   ...
   * {@link java.sql.DatabaseMetaData#supportsResultSetConcurrency(int, int)}, except that the JdbcOdbc bridge in JDK
   * 1.6 overstates its abilities. See bug 1690406.
   *
  ...
  /**
   * Returns the database for this Dialect, or {@link mondrian.spi.Dialect.DatabaseProduct#UNKNOWN} if the database is
  ...
  /**
   * Assembles and returns a string containing any hints that should be appended after the FROM clause in a SELECT
   * statement, based on any hints provided.  Any unrecognized or unsupported hints will be ignored.
   *
   * @param buf   The Stringbuffer to which the dialect-specific syntax for any relevant table hints may be appended.
   *              Must not be null.
   ...
   * dialects to be cached and reused in environments where connections are allocated from a pool based on the same
   * data
   * source.</p>
   *
   * <p>Data sources are deemed 'equal' by the same criteria used by Java
   * collections, namely the {@link Object#equals(Object)} and {@link Object#hashCode()} methods.</p>
   *
   * @return Whether this dialect can be used for other connections created from the same data source
   * @see mondrian.spi.DialectFactory#createDialect(javax.sql.DataSource, java.sql.Connection)
   */
  boolean allowsDialectSharing();

  /**
   * Returns whether the database currently permits queries to include in the SELECT clause expressions that are not
   * listed in the GROUP BY clause. The SQL standard allows this if the database can deduce that the expression is
   * functionally dependent on columns in the GROUP BY clause.
   *
   * <p>For example, {@code SELECT empno, first_name || ' ' || last_name FROM
   * emps GROUP BY empno} is valid because {@code empno} is the primary key of the {@code emps} table, and
   * therefore all
   * columns are dependent on it. For a given value of {@code empno}, {@code first_name || ' ' || last_name} has a
   * unique value.
   *
   * <p>Most databases do not, MySQL is an example of one that does (if the
   * functioality is enabled).
   *
   * @return Whether this Dialect allows SELECT clauses to contain columns that are not in the GROUP BY clause
   */
  boolean allowsSelectNotInGroupBy();

  /**
   * Returns whether this dialect supports "ANSI-style JOIN syntax", {@code FROM leftTable JOIN rightTable ON
   * conditon}.
   *
  ...
  /**
   * Some databases, like Greenplum, don't include nulls as part of the results of a COUNT sql call. This allows
   * dialects to wrap the count expression in something before it is used in the query.
   *
  ...
  /**
   * Must generate a String representing a regular expression match operation between a string literal and a Java
   * regular expression. The string literal might be a column identifier or some other identifier, but the
   * implementation must presume that it is already escaped and fit for use. The regular expression is not escaped and
   * must be adapted to the proper dialect rules.
   * <p>Postgres / Greenplum example:
   * <p><code>
   * generateRegularExpression( "'foodmart'.'customer_name'", "(?i).*oo.*") -> 'foodmart'.'customer_name' ~
   * "(?i).*oo.*"
   * </code></p>
   * <p>Oracle example:
   * <p><code>
   * generateRegularExpression( "'foodmart'.'customer_name'", ".*oo.*") -> REGEXP_LIKE('foodmart'.'customer_name',
   * ".*oo.*")
   * </code></p>
   *
   * <p>Dialects are allowed to return null if the dialect cannot
   * convert that particular regular expression into something that the database would support.</p>
   *
   * @param source     A String identifying the column to match against.
   * @param javaRegExp A Java regular expression to match against.
   * @return A dialect specific matching operation, or null if the dialect cannot convert that particular regular
   * expression into something that the database would support.
   */
   ...
   * {@code mondrian.statistics.providers.PRODUCT} where product is the current dialect's product name (for example
   * "MYSQL"). If that property has no value, looks at the property {@code mondrian.statistics.providers}. The
   * property
   * value should be a comma-separated list of names of classes that implement the {@link StatisticsProvider} interface.
   * For each statistic required, Mondrian will call the method each statistics provider in turn, until one of them
   * returns a non-negative value.</p>
   */
   ...
   * encapsulated in implementing methods.  For example, if a dialect has implicit rules involving scale or precision,
   * they should be handled within this method so the client can simply retrieve the "best fit" SqlStatement.Type for
   * the column.</p>
   *
   ...
   * differently for different databases. However, since the capabilities of a database can change between versions, it
   * is recommended that conditional code is in terms of capabilities methods in {@link mondrian.spi.Dialect}.
   *
   * <p>Because there are so many differences between various versions and
   * ports of DB2, we represent them as 3 separate products. If you want to treat them all as one product, note
   * that the
   * {@link #getFamily()} method for {@link #DB2_AS400} and {@link #DB2_OLD_AS400} returns {@link #DB2}.
   */
  enum DatabaseProduct {
     ...
     * {@link #DB2}; for all other database products, returns the same product.
     *
