 ...
 */
class SqlMemberSource implements MemberReader, MemberBuilder {
  ...
  private final boolean assignOrderKeys;
  ...
  public RolapMember getMemberByKey( RolapLevel level, List<Comparable> keyValues ) {
    ...
    final List<RolapMember> list =
      getMembersInLevel( level, new MemberKeyConstraint( columnList, datatypeList, keyValues ) );
  ...
  private int getMemberCount( RolapLevel level, DataSource dataSource ) {
    ...
    final SqlStatement stmt = RolapUtil.executeQuery( dataSource, sql,
      new Locus( Locus.peek().execution, "SqlMemberSource.getLevelMemberCount",
  ...
  private String makeLevelMemberCountSql( RolapLevel level, DataSource dataSource, boolean[] mustCount ) {
    ...
    if ( !sqlQuery.getDialect().allowsFromQuery() ) {
      ...
      SqlQuery outerQuery =
        SqlQuery.newQuery( dataSource, "while generating query to count members in level " + level );
      outerQuery.addSelect( "count(*)", null );
      // Note: the "init" is for Postgres, which requires FROM-queries to have an alias
  ...
  private List<RolapMember> getMembers( DataSource dataSource ) {
    ...
    SqlStatement stmt = RolapUtil.executeQuery( dataSource, sql, types, 0, 0,
      new SqlStatement.StatementLocus( null, "SqlMemberSource.getMembers", "while building member cache",
    ...
    try {
      ...
      Map<MemberKey, RolapMember> map = new HashMap<>();
...
*/
  ...
  private int countMemberChildren( RolapMember parentMember, List<RolapMember> children,
    ...
    SqlQuery newQuery = SqlQuery.newQuery( dataSource,
      "while generating query to retrieve children of " + "parent/child hierarchy member " + parentMember );
    ...
    SqlStatement stmt = RolapUtil.executeQuery( dataSource, sql, types, 0, 0,
      new SqlStatement.StatementLocus( Locus.peek().execution, "SqlMemberSource.getMemberChildren",
        "while building member cache", SqlStatementEvent.Purpose.TUPLES, 0 ), -1, -1, null );
    ...
    try {
      ...
      RolapUtil.LOGGER.error( "Exception: The query [" + sql + "] returned no values" );
    } catch ( SQLException e ) {
      throw stmt.handle( e );
    } finally {
      stmt.close();
  ...
  private Pair<String, List<SqlStatement.Type>> makeKeysSql( DataSource dataSource ) {
    SqlQuery sqlQuery = SqlQuery.newQuery( dataSource, "while generating query to retrieve members of " + hierarchy );
    RolapLevel[] levels = (RolapLevel[]) hierarchy.getLevels();
    ...
    for ( RolapLevel level : levels ) {
      ...
      for ( RolapProperty property : properties ) {
        ...
        // Some dialects allow us to eliminate properties from the group by that are functionally dependent on the
        // level value
  ...
  public List<RolapMember> getMembersInLevel( RolapLevel level, TupleConstraint constraint ) {
    ...
    final TupleReader tupleReader =
      isHighCardinality ? new HighCardSqlTupleReader( constraint ) : new SqlTupleReader( constraint );
   ...
   */
  Pair<String, List<SqlStatement.Type>> makeChildMemberSql( RolapMember member, DataSource dataSource,
                                                            MemberChildrenConstraint constraint ) {
    return makeChildMemberSqlQuery( member, dataSource, constraint ).toSqlAndTypes();
  }

  SqlQuery makeChildMemberSqlQuery( RolapMember member, DataSource dataSource, MemberChildrenConstraint constraint ) {
  ...
  SqlQuery makeChildMemberSqlQuery( RolapMember member, DataSource dataSource, MemberChildrenConstraint constraint,
    ...
    SqlQuery sqlQuery =
      SqlQuery.newQuery( dataSource, "while generating query to retrieve children of member " + member );
  ...
  private static AggStar chooseAggStar( MemberChildrenConstraint constraint, RolapMember member ) {
    ...
    // Convert global ordinal to cube based ordinal (the 0th dimension is always [Measures])

    // Expand calculated so we don't miss their bitkeys
    final Member[] members =
      SqlConstraintUtils.expandSupportedCalculatedMembers( Arrays.asList( evaluator.getNonAllMembers() ), evaluator )
        .getMembersArray();
  ...
  public Map<? extends Member, Access> getMemberChildren( RolapMember parentMember, List<RolapMember> children,
                                                          MemberChildrenConstraint constraint ) {
    // allow parent child calculated members through this fixes the non closure parent child hierarchy bug
  ...
  private void getMemberChildren2( RolapMember parentMember, List<RolapMember> children,
    ...
    SqlStatement stmt = RolapUtil.executeQuery( dataSource, sql, types, 0, 0,
      new SqlStatement.StatementLocus( Locus.peek().execution, "SqlMemberSource.getMemberChildren",
        "while building member cache", SqlStatementEvent.Purpose.TUPLES, 0 ), -1, -1, null ); try {
      int limit = MondrianProperties.instance().ResultLimit.get();
      ...
      while ( resultSet.next() ) {
        ...
        if ( !childLevel.getOrdinalExp().equals( childLevel.getKeyExp() ) ) {
          ...
          if ( prevValue != null && !Util.equals( prevValue, ordinal ) ) {
            LOGGER.error(
              "Column expression for " + member.getUniqueName() + " is inconsistent with ordinal or caption expression."
  ...
  public RolapMember makeMember( RolapMember parentMember, RolapLevel childLevel, Object value, Object captionValue,
    ...
    if ( parentChild ) {
      ...
      // children.
      member = childLevel.hasClosedPeer() ? new RolapParentChildMember( parentMember, rolapChildLevel, value, member ) :
        new RolapParentChildMemberNoClosure( parentMember, rolapChildLevel, value, member );
    }
    ...
    for ( int j = 0; j < properties.length; j++ ) {
      ...
      member.setProperty( property.getName(), getPooledValue( accessors.get( columnOffset + j ).get() ) );
    }
  ...
  public RolapMember allMember() {
    final RolapHierarchy rolapHierarchy =
      hierarchy instanceof RolapCubeHierarchy ? ( (RolapCubeHierarchy) hierarchy ).getRolapHierarchy() : hierarchy;
   ...
   * 'All' level).
   */
  private Pair<String, List<SqlStatement.Type>> makeChildMemberSqlPCRoot( RolapMember member ) {
    return makeChildMemberSqlPCRootQuery( member ).toSqlAndTypes();
  }

  private SqlQuery makeChildMemberSqlPCRootQuery( RolapMember member ) {
  ...
  private void addLevel( SqlQuery sqlQuery, RolapLevel level, boolean group ) {
    ...
    final String keyAlias = group ? sqlQuery.addSelectGroupBy( keySql, level.getInternalType() ) :
    ...
    if ( !orderSql.equals( keySql ) ) {
      final String orderAlias =
        group ? sqlQuery.addSelectGroupBy( orderSql, null ) : sqlQuery.addSelect( orderSql, null );
    ...
    for ( RolapProperty property : properties ) {
      ...
      if ( group && ( !sqlQuery.getDialect().allowsSelectNotInGroupBy() || !property.dependsOnLevelValue() ) ) {
        sqlQuery.addGroupBy( s, alias );
      }
    }
  }
  ...
  private static class RolapParentChildMember extends RolapMemberBase {
    ...
    public RolapParentChildMember( RolapMember parentMember, RolapLevel childLevel, Object value,
