...
*/
    ...
    private int countMemberChildren(RolapMember parentMember, List<RolapMember> children, MemberChildrenConstraint constraint) {
        ...
        if ( parentLevel.isParentChild() ) {
            ...
            if ( childLevel.isParentChild() ) {
                query = makeChildMemberSql_PCRootQuery( parentMember );
                parentChild = true;
            } else {
    ...
    private Pair<String, List<SqlStatement.Type>> makeChildMemberSql_PCRoot(
        RolapMember member)
    {
        return makeChildMemberSql_PCRootQuery(member).toSqlAndTypes();
    }
    private SqlQuery makeChildMemberSql_PCRootQuery(
      RolapMember member)
    {
        SqlQuery sqlQuery =
          SqlQuery.newQuery(
