...
public class RolapCubeHierarchy extends RolapHierarchy {
  private final boolean cachingEnabled = MondrianProperties.instance().EnableRolapCubeMemberCache.get();
  ...
  /**
   * Length of prefix to be removed when translating member unique names, or 0 if no translation is necessary.
   */
  private final int removePrefixLength;

  // redundant copy of {@link #levels} with tighter type
  ...
  public RolapCubeHierarchy( RolapCubeDimension cubeDimension, MondrianDef.CubeDimension cubeDim,
                             RolapHierarchy rolapHierarchy, String subName, int ordinal ) {
    this( cubeDimension, cubeDim, rolapHierarchy, subName, ordinal, null );
  }
  ...
  public RolapCubeHierarchy( RolapCubeDimension cubeDimension, MondrianDef.CubeDimension cubeDim,
    ...
    // this relation should equal the name of the new dimension table The null member belongs to a level with very
    // similar properties to the 'all' level.
    ...
    usingCubeFact =
      ( factCube == null || factCube.getFact() == null || factCube.getFact().equals( rolapHierarchy.getRelation() ) );
    ...
    if ( !cubeIsVirtual && !usingCubeFact ) {
      ...
      currentRelation = this.cubeDimension.getCube().getStar()
        .getUniqueRelation( rolapHierarchy.getRelation(), usage.getForeignKey(),
          ( (MondrianDef.Column) usage.getJoinExp() ).getColumnName(), usage.getJoinTable().getAlias() );
    } else {
    ...
    for ( int i = 0; i < rolapHierarchy.getLevels().length; i++ ) {
      ...
      if ( i == 0 && rolapHierarchy.getAllMember() != null ) {
        ...
        if ( hasAll() ) {
          ...
          // create an all level if one doesn't normally exist in the hierarchy
    ...
    // Compute whether the unique names of members of this hierarchy are different from members of the underlying
    // hierarchy. If so, compute the length of the prefix to be removed before this hierarchy's unique name is added.
    // For example, if this.uniqueName is "[Ship Time]" and rolapHierarchy.uniqueName is "[Time]", remove
    // prefixLength will be length("[Ship Time]") = 11.
  ...
  /**
   * Applies a prefix to a caption or description of a hierarchy in a shared dimension. Ensures that if a dimension
   * is used more than once in the same cube then the hierarchies are distinguishable.
   *
   * <p>For example, if the [Time] dimension is imported as [Order Time] and [Ship Time], then the [Time].[Weekly]
   * hierarchy would have caption "Order Time.Weekly caption" and description "Order Time.Weekly description".
   *
   * <p>If the dimension usage has a caption, it overrides.
   *
   * <p>If the dimension usage has a null name, or the name is the same as the dimension, and no caption, then no
   * prefix is applied.
   *
   ...
   *
   * <p>This method may be simplified when we obsolete {@link mondrian.rolap.HierarchyUsage}.
   *
   * @param oldrel Original relation, as defined in the schema
   * @param newrel New star relation, generated by RolapStar, canonical, and shared between all cubes with similar
   *               structure
   */
  protected void extractNewAliases( MondrianDef.RelationOrJoin oldrel, MondrianDef.RelationOrJoin newrel ) {
    if ( oldrel == null && newrel == null ) {
      return;
    } else if ( oldrel instanceof MondrianDef.Relation && newrel instanceof MondrianDef.Relation ) {
      aliases.put( ( (MondrianDef.Relation) oldrel ).getAlias(), ( (MondrianDef.Relation) newrel ).getAlias() );
  ...
  public boolean equals( Object o ) {
    ...
    return cubeDimension.equals( that.cubeDimension ) && getUniqueName().equals( that.getUniqueName() );
  }
  ...
  /**
   * The currentRelation object is derived from the shared relation object it is generated via the RolapStar object,
   * and contains unique aliases for its particular join path
   *
  ...
  public final RolapCubeMember getDefaultMember() {
    if ( currentDefaultMember == null ) {
      ...
      currentDefaultMember = bootstrapLookup( (RolapMember) rolapHierarchy.getDefaultMember() );
    }
  ...
  /**
   * Looks up a {@link RolapCubeMember} corresponding to a {@link RolapMember} of the underlying hierarchy. Safe to
   * be called while the hierarchy is initializing.
   *
  ...
  private RolapCubeMember bootstrapLookup( RolapMember rolapMember ) {
    RolapCubeMember parent = rolapMember.getParentMember() == null ? null :
      ( rolapMember.getParentMember().isAll() ? currentAllMember : bootstrapLookup( rolapMember.getParentMember() ) );
  ...
  public Member getNullMember() {
    ...
    if ( currentNullMember == null ) {
      currentNullMember = new RolapCubeMember( null, (RolapMember) rolapHierarchy.getNullMember(), currentNullLevel );
    }
  ...
  public void setDefaultMember( Member defaultMeasure ) {
    ...
    RolapCubeLevel level = new RolapCubeLevel( (RolapLevel) rolapHierarchy.getDefaultMember().getLevel(), this );
    currentDefaultMember = new RolapCubeMember( null, (RolapMember) rolapHierarchy.getDefaultMember(), level );
  }
   ...
   *
   * <p>For example, if the shared hierarchy is [Time].[Quarterly] and the hierarchy usage is [Ship Time]
   * .[Quarterly], then [Time].[1997].[Q1] would be translated to [Ship Time].[Quarerly].[1997].[Q1].
   *
  ...
  final String convertMemberName( String memberUniqueName ) {
    if ( removePrefixLength > 0 && !memberUniqueName.startsWith( uniqueName ) ) {
  ...
  private static RolapCubeMember createAncestorMembers( RolapCubeHierarchyMemberReader memberReader,
                                                        RolapCubeLevel level, RolapMember member ) {
    if ( member == null ) {
      return null;
    }
  ...
  public interface RolapCubeHierarchyMemberReader extends MemberReader {
    RolapCubeMember lookupCubeMember( final RolapCubeMember parent, final RolapMember member,
  ...
  /******
   ...
   This is the shared cache across shared hierarchies. This member cache only contains members loaded by non-cube
   specific member lookups. This cache should only contain RolapMembers, not RolapCubeMembers

   - RolapCubeHierarchy.RolapCubeHierarchyMemberReader.rolapCubeCacheHelper ->
   This cache contains the RolapCubeMember objects, which are cube specific wrappers of shared members.

   - RolapCubeHierarchy.RolapCubeHierarchyMemberReader.SmartCacheHelper ->
   This is the inherited shared cache from SmartMemberReader, and is used when a join with the fact table is
   necessary, aka a SqlContextConstraint is used. This cache may be redundant with rolapCubeCacheHelper.

   - A Special note regarding RolapCubeHierarchyMemberReader.cubeSource -
   This class was required for the special situation getMemberBuilder() method call from RolapNativeSet.  This class
   utilizes both the rolapCubeCacheHelper class for storing RolapCubeMembers, and also the
   ...
   *
   * <p>Synchronization. Most synchronization takes place within SmartMemberReader. All synchronization is done on
   * the cacheHelper object.
   */
  public class CacheRolapCubeHierarchyMemberReader extends SmartMemberReader implements RolapCubeHierarchyMemberReader {
    ...
    private final boolean enableCache = MondrianProperties.instance().EnableRolapCubeMemberCache.get();
    ...
    public CacheRolapCubeHierarchyMemberReader() {
      ...
      cubeSource = new RolapCubeSqlMemberSource( this, RolapCubeHierarchy.this, rolapCubeCacheHelper, cacheHelper );
    ...
    @Override
    protected void readMemberChildren( List<RolapMember> parentMembers, List<RolapMember> children,
      ...
      // extract RolapMembers from their RolapCubeMember objects populate lookup for reconnecting parents and children
      ...
      // get member children from shared member reader if possible, if not get them from our own source
      ...
      if ( joinReq ) {
        ...
        rolapHierarchy.getMemberReader().getMemberChildren( rolapParents, rolapChildren, constraint );
      }

      // now lookup or create RolapCubeMember
      for ( RolapMember currMember : rolapChildren ) {
        RolapCubeMember parent = lookup.get( currMember.getParentMember().getUniqueName() );
      ...
      // Put them in a temporary hash table first. Register them later, when we know their size (hence their 'cost'
      // to the cache pool).
      ...
      // note that this stores RolapCubeMembers in our cache, which also stores RolapMembers.

      for ( RolapMember child : children ) {
        // todo: We could optimize here. If members.length is small, it's more efficient to drive from members,
        //  rather than hashing children.length times. We could also exploit the fact that the result is sorted by
        //  ordinal and therefore, unless the "members" contains members from different levels, children of the same
        ...
        if ( cacheList == null ) {
          // The list is null if, due to dropped constraints, we now have a children list of a member we didn't
          // explicitly ask for it. Adding it to the cache would be viable, but let's ignore it.
          continue;
    ...
    public Map<? extends Member, Access> getMemberChildren( List<RolapMember> parentMembers, List<RolapMember> children,
      ...
      synchronized ( cacheHelper ) {
        ...
        for ( RolapMember parentMember : parentMembers ) {
          List<RolapMember> list = rolapCubeCacheHelper.getChildrenFromCache( parentMember, constraint );
    ...
    public List<RolapMember> getMembersInLevel( RolapLevel level, TupleConstraint constraint ) {
      synchronized ( cacheHelper ) {
        ...
        // if a join is required, we need to pass in the RolapCubeLevel vs. the regular level
        boolean joinReq = ( constraint instanceof SqlContextConstraint );
        List<RolapMember> list;
        final RolapCubeLevel cubeLevel = (RolapCubeLevel) level;

        if ( !joinReq ) {
          list = rolapHierarchy.getMemberReader().getMembersInLevel( cubeLevel.getRolapLevel(), constraint );
        } else {
        ...
        for ( RolapMember member : list ) {
          ...
          // REVIEW: disabled, to see what happens. if this code is for performance, we should check level.isAll at
          // the top of the method; if it is for correctness, leave the code in
          RolapCubeMember cubeMember = lookupCubeMemberWithParent( member, cubeLevel );
          newlist.add( cubeMember );
        }

        rolapCubeCacheHelper.putChildren( level, constraint, newlist );
    ...
    private RolapCubeMember lookupCubeMemberWithParent( RolapMember member, RolapCubeLevel cubeLevel ) {
      ...
      if ( parentMember == null ) {
        ...
        // In parent-child hierarchies, a member's parent may be in the same level.
        final RolapCubeLevel parentLevel =
          parentMember.getLevel() == member.getLevel() ? cubeLevel : cubeLevel.getParentLevel();
    ...
    public RolapMember getMemberByKey( RolapLevel level, List<Comparable> keyValues ) {
      synchronized ( cacheHelper ) {
        final RolapMember member = super.getMemberByKey( level, keyValues );
        return createAncestorMembers( this, (RolapCubeLevel) level, member );
      }
    }
    ...
    protected void checkCacheStatus() {
      synchronized ( cacheHelper ) {
        ...
        //   - local regular RolapMember cache, used when cube specific joins occur

        if ( cacheHelper.getChangeListener() != null
          && cacheHelper.getChangeListener().isHierarchyChanged( getHierarchy() ) ) {
          cacheHelper.flushCache();
          rolapCubeCacheHelper.flushCache();

          if ( rolapHierarchy.getMemberReader() instanceof SmartMemberReader ) {
            SmartMemberReader smartMemberReader = (SmartMemberReader) rolapHierarchy.getMemberReader();
  ...
  /**
   * Same as {@link RolapCubeHierarchyMemberReader} but without caching anything.
   */
  public class NoCacheRolapCubeHierarchyMemberReader extends NoCacheMemberReader
     ...
     * . The inherited cacheHelper object contains non-shared hierarchy RolapMembers. non-shared hierarchy
     * RolapMembers are created when a member lookup involves the Cube's fact table.
     */
    protected MemberCacheHelper rolapCubeCacheHelper;
    ...
    public NoCacheRolapCubeHierarchyMemberReader() {
      ...
      cubeSource =
        new RolapCubeSqlMemberSource( this, RolapCubeHierarchy.this, rolapCubeCacheHelper, new MemberNoCacheHelper() );
    ...
    protected void readMemberChildren( List<RolapMember> parentMembers, List<RolapMember> children,
      ...
      // extract RolapMembers from their RolapCubeMember objects populate lookup for reconnecting parents and children
      final List<RolapCubeMember> parentRolapCubeMemberList = Util.cast( parentMembers );
    ...
    public List<RolapMember> getMembersInLevel( final RolapLevel level, TupleConstraint constraint ) {
      ...
      if ( !joinReq ) {
        list =
          rolapHierarchy.getMemberReader().getMembersInLevel( ( (RolapCubeLevel) level ).getRolapLevel(), constraint );
      } else {
        list = super.getMembersInLevel( level, constraint );
      }
      ...
      return new UnsupportedList<RolapMember>() {
        ...
        private RolapMember mutate( final RolapMember member ) {
          ...
          if ( member.getParentMember() != null ) {
            parent = createAncestorMembers( NoCacheRolapCubeHierarchyMemberReader.this,
              (RolapCubeLevel) level.getParentLevel(), member.getParentMember() );
          }
  ...
  public static class RolapCubeSqlMemberSource extends SqlMemberSource {
    ...
    public RolapCubeSqlMemberSource( RolapCubeHierarchyMemberReader memberReader, RolapCubeHierarchy hierarchy,
    ...
    @Override
    public RolapMember makeMember( RolapMember parentMember, RolapLevel childLevel, Object value, Object captionValue,
      ...
      RolapMember member =
        super.makeMember( parent, childCubeLevel.getRolapLevel(), value, captionValue, parentChild, stmt, key,
          columnOffset );
