...
public class BasicQueryTest extends FoodMartTestCase {
   ...
   * where the ordering of the WITH MEMBER would determine whether or not the member would be found in the cube. This
   * test would fail but the previous one would work ok.
   */
   ...
   * causes an internal error ("value not found") when the cell's formatted value is retrieved.
   */
   ...
   * definition does not include an explicit count measure, is flagged 'not visible' but is still correctly returned
   * from [Measures].Members.
   */
  ...
  public void testUnparse() {
    ...
    TestContext.assertEqualsVerbose(
      "with member [Measures].[Rendite] as '(([Measures].[Store Sales] - [Measures].[Store Cost]) / [Measures].[Store"
        ...
        + "100) > Parameter(\"UpperLimit\", NUMERIC, 151, \"Obere Grenze\")), "
        + "\"|#.00%|arrow='up'\", "
   ...
   * budget information that can be displayed by store. The Sales cube in the FoodMart 2000 database contains sales
   * information that can be displayed by store. Since no virtual cube exists in the FoodMart 2000 database that joins
   * the Sales and Budget cubes together, comparing the two sets of figures would be difficult at best.
   *
   * <p><b>Note<b> In many situations a virtual cube can be used to integrate
   * data from multiple cubes, which will often provide a simpler and more efficient solution than the LookupCube
   * function. This example uses the LookupCube function for purposes of illustration.
   *
   * <p>The following MDX query, however, uses the LookupCube function to
   * retrieve unit sales information for each store from the Sales cube, presenting it side by side with the budget
   * information from the Budget cube.
   */
   ...
   * but some basic forms of basket analysis can be handled through the use of MDX queries.
   *
   * <p>For example, one method of basket analysis groups customers based on
   * qualification. In the following example, a qualified customer is one who has more than $10,000 in store sales or
   * more than 10 unit sales. The following table illustrates such a report, run against the Sales cube in FoodMart 2000
   * with qualified customers grouped by the Country and State Province levels of the Customers dimension. The count and
   * store sales total of qualified customers is represented by the Qualified Count and Qualified Sales columns,
   * respectively.
   *
   * <p>To accomplish this basic form of basket analysis, the following MDX
   * query constructs two calculated members. The first calculated member uses the MDX Count, Filter, and Descendants
   * functions to create the Qualified Count column, while the second calculated member uses the MDX Sum, Filter, and
   ...
   * together to screen out non-qualified customers. Once screened out, the Sum and Count MDX functions can then be used
   * to provide aggregation data only on qualified customers.
   */
   ...
   * string comparison and manipulation functions, for example, for finding substrings in strings or for supporting
   * case-insensitive string comparisons. However, since MDX can take advantage of external function libraries, this
   * question is easily resolved using string manipulation and comparison functions from the Microsoft Visual Basic for
   * Applications (VBA) external function library.
   *
   * <p>For example, you want to report the unit sales of all fruit-based
   * products -- not only the sales of fruit, but canned fruit, fruit snacks, fruit juices, and so on. By using the
   * LCase and InStr VBA functions, the following results are easily accomplished in a single MDX query, without complex
   * set construction or explicit member names within the query.
   *
   * <p>The following MDX query demonstrates how to achieve the results
   * displayed in the previous table. For each member in the Product dimension, the name of the member is converted to
   * lowercase using the LCase VBA function. Then, the InStr VBA function is used to discover whether or not the name
   * contains the word "fruit". This information is used to then construct a set, using the Filter MDX function, from
   * only those members from the Product dimension that contain the substring "fruit" in their names.
   */
   ...
   * unit sales for each store in a given city, state, and country, organized along the Sales dimension. A report is
   * requested to show, for California, the percentage of total unit sales attained by each city with a store. The
   * results are illustrated in the following table.
   *
   * <p>Because the parent of a member is typically another, aggregated
   * member in a regular dimension, this is easily achieved by the construction of a calculated member, as demonstrated
   * in the following MDX query, using the CurrentMember and Parent MDX functions.
   */
   ...
   * business reporting purposes. However, since aggregations are handled in a hierarchical fashion, cumulative sums
   * present some unique challenges in Analysis Services.
   *
   ...
   * views of employee count in all stores and cities in California, sorted by employee count. The first column shows
   * the aggregated counts for each store and city, while the second column shows aggregated counts for each store, but
   * cumulative counts for each city.
   *
   * <p>The cumulative number of employees for San Diego represents the value
   * of both Los Angeles and San Diego, the value for Beverly Hills represents the cumulative total of Los Angeles, San
   * Diego, and Beverly Hills, and so on.
   *
   * <p>Since the members within the state of California have been ordered
   * from highest to lowest number of employees, this form of cumulative sum measure provides a form of pareto analysis
   * within each state.
   *
   * <p>To support this, the Order function is first used to reorder members
   * accordingly for both the Rank and Head functions. Once reordered, the Rank function is used to supply the ranking
   * of each tuple within the reordered set of members, progressing as each member in the Store dimension is examined.
   * The value is then used to determine the number of tuples to retrieve from the set of reordered members using the
   * Head function. Finally, the retrieved members are then added together using the Sum function to obtain a cumulative
   * sum. The following MDX query demonstrates how all of this works in concert to provide cumulative sums.
   *
   * <p>As an aside, a named set cannot be used in this situation to replace
   * the duplicate Order function calls. Named sets are evaluated once, when a query is parsed -- since the set can
   * change based on the fact that the set can be different for each store member because the set is evaluated for the
   * children of multiple parents, the set does not change with respect to its use in the Sum function. Since the named
   * set is only evaluated once, it would not satisfy the needs of this query.
   */
   ...
   * clause of a SQL statement is an essential tool for constructing business queries. However, the WHERE clause of an
   * MDX statement serves a slightly different purpose, and understanding how the WHERE clause is used in MDX can assist
   * in constructing such business queries.
   *
   * <p>The WHERE clause in MDX is used to further restrict the results of
   * an MDX query, in effect providing another dimension on which the results of the query are further sliced. As such,
   * only expressions that resolve to a single tuple are allowed. The WHERE clause implicitly supports a logical AND
   * operation involving members across different dimensions, by including the members as part of a tuple. To support
   * logical AND operations involving members within a single dimensions, as well as logical OR operations, a calculated
   * member needs to be defined in addition to the use of the WHERE clause.
   *
   * <p>For example, the following MDX query illustrates the use of a
   * calculated member to support a logical OR. The query returns unit sales by quarter and year for all food and drink
   * related products sold in 1997, run against the Sales cube in the FoodMart 2000 database.
   *
   * <p>The calculated member simply adds the values of the Unit Sales
   * sure for the Food and the Drink levels of the Product dimension together. The WHERE clause is then used to restrict
   * return of information only to the calculated member, effectively implementing a logical OR to return information
   * for all time periods that contain unit sales values for either food, drink, or both types of products.
   *
   * <p>You can use the Aggregate function in similar situations where all
   * measures are not aggregated by summing. To return the same results in the above example using the Aggregate
   * function, replace the definition for the calculated member with this definition:
   *
   * <blockquote>
   ...
   * techniques. If the members used to construct the logical AND reside on different dimensions, all that is required
   * is a WHERE clause that uses a tuple representing all involved members. The following MDX query uses a WHERE clause
   * that effectively restricts the query to retrieve unit sales for drink products in the USA, shown by quarter and
   * year for 1997.
   */
   ...
   * logical AND operator, in which all unit sales for 1997 are returned only for drink products and only for those sold
   * in stores in the USA.
   *
   * <p>If the members used to construct the logical AND condition reside on
   * the same dimension, you can use a calculated member or a named set to filter out the unwanted members, as
   * demonstrated in the following MDX query.
   *
   * <p>The named set, [Good AND Pearl Stores], restricts the displayed unit
   * sales totals only to those stores that have sold both Good products and Pearl products.
   */
   ...
   * information to members in a dimension. However, getting that information out can be confusing -- member properties
   * are not readily apparent in a typical MDX query.
   *
   * <p>Member properties can be retrieved in one of two ways. The easiest
   * and most used method of retrieving member properties is to use the DIMENSION PROPERTIES MDX statement when
   * constructing an axis in an MDX query.
   *
   * <p>For example, a member property in the Store dimension in the FoodMart
   * 2000 database details the total square feet for each store. The following MDX query can retrieve this member
   * property as part of the returned cellset.
   */
   ...
   * for most client applications, the member property is not readily apparent. If the previous MDX query is executed in
   * the MDX sample application shipped with SQL Server 2000 Analysis Services, for example, you must double-click the
   * name of the member in the grid to open the Member Properties dialog box, which displays all of the member
   * properties shipped as part of the cellset, including the [Store].[Store Name].[Store Sqft] member property.
   *
   * <p>The other method of retrieving member properties involves the creation
   * of a calculated member based on the member property. The following MDX query brings back the total square feet for
   * each store as a measure, included in the COLUMNS axis.
   *
   * <p>The [Store SqFt] measure is constructed with the Properties MDX
   * function to retrieve the [Store SQFT] member property for each member in the Store dimension. The benefit to this
   * technique is that the calculated member is readily apparent and easily accessible in client applications that do
   * not support member properties.
   */
   ...
   * Analysis Services is no exception. Several functions exist that support drilling up and down the hierarchy of
   * dimensions within a cube. Typically, drilling up and down the hierarchy is done one level at a time; think of this
   * functionality as a zoom feature for OLAP data.
   *
   * <p>There are times, though, when the need to drill down more than one
   * level at the same time, or even skip levels when displaying information about multiple levels, exists for a
   * business scenario.
   *
   * <p>For example, you would like to show report results from a query of
   * the Sales cube in the FoodMart 2000 sample database showing sales totals for individual cities and the subtotals
   * for each country, as shown in the following table.
   *
   * <p>The Customers dimension, however, has Country, State Province, and
   * City levels. In order to show the above report, you would have to show the Country level and then drill down two
   * levels to show the City level, skipping the State Province level entirely.
   *
   * <p>However, the MDX ToggleDrillState and DrillDownMember functions
   * provide drill down functionality only one level below a specified set. To drill down more than one level below a
   * specified set, you need to use a combination of MDX functions, including Descendants, Generate, and Except. This
   * technique essentially constructs a large set that includes all levels between both upper and lower desired levels,
   * then uses a smaller set representing the undesired level or levels to remove the appropriate members from the
   * larger set.
   *
   * <p>The MDX Descendants function is used to construct a set consisting of
   * the descendants of each member in the Customers dimension. The descendants are determined using the MDX Descendants
   * function, with the descendants of the City level and the level above, the State Province level, for each member of
   * the Customers dimension being added to the set.
   *
   * <p>The MDX Generate function now creates a set consisting of all members
   * at the Country level as well as the members of the set generated by the MDX Descendants function. Then, the MDX
   * Except function is used to exclude all members at the State Province level, so the returned set contains members at
   * the Country and City levels.
   *
   * <p>Note, however, that the previous MDX query will still order the
   * members according to their hierarchy. Although the returned set contains members at the Country and City levels,
   * the Country, State Province, and City levels determine the order of the members.
   */
   ...
   * level of a dimension within a cube are needed, but information about other levels within the same dimension may
   * also be required to satisfy a specific business scenario.
   *
   * <p>For example, a report that shows the unit sales for the store with
   * the highest unit sales from each country is needed for marketing purposes. The following table provides an example
   * of this report, run against the Sales cube in the FoodMart 2000 sample database.
   *
   * <p>This looks simple enough, but the Country Name column provides
   * unexpected difficulty. The values for the Store Country column are taken from the Store Country level of the Store
   * dimension, so the Store Country column is constructed as a calculated member as part of the MDX query, using the
   * MDX Ancestor and Name functions to return the country names for each store.
   *
   * <p>A combination of the MDX Generate, TopCount, and Descendants
   * functions are used to create a set containing the top stores in unit sales for each country.
   */
  ...
  /**
   * <p>The MDX Descendants function is used to construct a set consisting of
   * only those members at the Store Name level in the Store dimension. Then, the MDX TopCount function is used to
   * return only the topmost store based on the Unit Sales measure. The MDX Generate function then constructs a set sed
   ...
   * not provide the desired results because non-related joins can occur. Since the Store Country and Store Name levels
   * are within the same dimension, they cannot be cross-joined. Another dimension that provides the same regional
   * hierarchy structure, such as the Customers dimension, can be employed with the Crossjoin function. But, using this
   * technique can cause non-related joins and return unexpected results.
   *
   ...
   * in the Store dimension is cross-joined with all of the children of the Customers dimension, as shown in the
   * following table.
   *
   * <p>In this instance, the use of a calculated member to provide store
   * country names is easier to understand and debug than attempting to cross-join across unrelated members
   */
   ...
   * need to rank the members of a dimension according to their corresponding measure values. The Order MDX function
   * allows you to order a set based on a string or numeric expression evaluated against the members of a set. Combined
   * with other MDX functions, the Order function can support several different types of ranking.
   *
   * <p>For example, the Sales cube in the FoodMart 2000 database can be used
   * to show unit sales for each store. However, the business scenario requires a report that ranks the stores from
   * highest to lowest unit sales, individually, of nonconsumable products.
   *
   * <p>Because of the requirement that stores be sorted individually, the
   * erarchy must be broken (in other words, ignored) for the purpose of ranking the stores. The Order function is
   * capable of sorting within the hierarchy, based on the topmost level represented in the set to be sorted, or, by
   * breaking the hierarchy, sorting all of the members of the set as if they existed on the same level, with the same
   * parent.
   *
   ...
   * are needed at different levels in a dimension. One easy way to support such functionality is through the use of a
   * calculated measure, created as part of the query, which uses the MDX Descendants function in conjunction with one
   * of the MDX aggregation functions to provide results.
   *
   * <p>For example, the Warehouse cube in the FoodMart 2000 database
   * supplies the [Units Ordered] measure, aggregated through the Sum function. But, you would also like to see the
   * average number of units ordered per store. The following table demonstrates the desired results.
   *
   * <p>By using the following MDX query, the desired results can be
   * achieved. The calculated measure, [Average Units Ordered], supplies the average number of ordered units per store
   * by using the Avg, CurrentMember, and Descendants MDX functions.
   */
   ...
   * example, you then want to see the average number of units ordered for beer products in all of the stores in the
   * California area, the following MDX query can be executed with the same calculated measure.
   */
   ...
   * dimensions. However, there are times where a different aggregation function may be needed to represent a measure
   * for reporting purposes. Two basic cases involve aggregating a single dimension using a different aggregation
   * function than the one used for other dimensions.<ul>
   *
   ...
   * dimension specified in the cube. For instance, to create a calculated measure that contains the average, along a
   * time dimension, of measures aggregated as sums along other dimensions, the average of the aggregated measures must
   * be taken over the set of averaging time periods, constructed through the use of the Descendants MDX function.
   * Minimum and maximum values are more easily calculated through the use of the Min and Max MDX functions, also
   * combined with the Descendants function.
   *
   * <p>For example, the Warehouse cube in the FoodMart 2000 database
   * contains information on ordered and shipped inventory; from it, a report is requested to show the average number of
   * units shipped, by product, to each store. Information on units shipped is added on a monthly basis, so the
   * aggregated measure [Units Shipped] is divided by the count of descendants, at the Month level, of the current
   * member in the Time dimension. This calculation provides a measure representing the average number of units shipped
   * per month, as demonstrated in the following MDX query.
   */
   ...
   * OpeningPeriod and ClosingPeriod MDX functions specifically to support opening and closing period values.
   *
   * <p>For example, the Warehouse cube in the FoodMart 2000 database
   * contains information on ordered and shipped inventory; from it, a report is requested to show on-hand inventory at
   * the end of every month. Because the inventory on hand should equal ordered inventory minus shipped inventory, the
   * ClosingPeriod MDX function can be used to create a calculated measure to supply the value of inventory on hand, as
   * demonstrated in the following MDX query.
   */
   ...
   * use ranges of dates, but OLAP objects provide aggregated information in date levels.
   *
   * <p>Using the technique described here, you can establish date ranges in
   * MDX queries at the level of granularity provided by a time dimension. Date ranges cannot be established below the
   * granularity of the dimension without additional information. For example, if the lowest level of a time dimension
   * represents months, you will not be able to establish a two-week date range without other information. Member
   * perties can be added to supply specific dates for members; using such member properties, you can take advantage of
   * the date and time functions provided by VBA and Excel external function libraries to establish date ranges.
   *
   * <p>The easiest way to specify a static date range is by using the colon
   * (:) operator. This operator creates a naturally ordered set, using the members specified on either side of the
   * operator as the endpoints for the ordered set. For example, to specify the first six months of 1998 from the Time
   ...
   * Quarter, and Month levels. To add a six-month and nine-month total, two calculated members are created in the
   * following MDX query.
   */
   ...
   * rolling date ranges. All of these techniques tend to fall into two groups. The first group involves the use of
   * relative hierarchical functions to construct named sets or calculated members, and the second group involves the
   * use of absolute date functions from external function libraries to construct named sets or calculated members. Both
   * groups are applicable in different business scenarios.
   *
   * <p>In the first group of techniques, typically a named set is
   * constructed which contains a number of periods from a time dimension. For example, the following table illustrates
   * a 12-month rolling period, in which the figures for unit sales of the previous 12 months are shown.
   *
   * <p>The following MDX query accomplishes this by using a number of MDX
   * functions, including LastPeriods, Tail, Filter, Members, and Item, to construct a named set containing only those
   * members across all other dimensions that share data with the time dimension at the Month level. The example assumes
   * that there is at least one measure, such as [Unit Sales], with a value greater than zero in the current period. The
   * Filter function creates a set of months with unit sales greater than zero, while the Tail function returns the last
   * month in this set, the current month. The LastPeriods function, finally, is then used to retrieve the last 12
   ...
   * being queried, to support different calculations for members depending on the time period. The following example
   * includes the MDX IIf function, and is easy to use but difficult to maintain. This example works well for ad hoc
   * queries, but is not the ideal technique for client applications in a production environment.
   *
   * <p>For example, the following table illustrates a standard and dynamic
   * forecast of warehouse sales, from the Warehouse cube in the FoodMart 2000 database, for drink products. The
   * standard forecast is double the warehouse sales of the previous year, while the dynamic forecast varies from month
   * to month -- the forecast for January is 120 percent of previous sales, while the forecast for July is 260 percent
   * of previous sales.
   *
   * <p>The most flexible way of handling this type of report is the use of
   * nested MDX IIf functions to return a multiplier to be used on the members of the Products dimension, at the Drinks
   * level. The following MDX query demonstrates this technique.
   */
   ...
   * Time or Product dimensions to support such calculations, are not as flexible but are much more efficient. The
   * primary drawback to using such techniques is that the calculations are not easily altered for speculative analysis
   * purposes. For client applications, however, where the calculations are static or slowly changing, using a member
   * property is an excellent way of supplying such functionality to clients while keeping maintenance of calculation
   * variables at the server level. The same MDX query, for example, could be rewritten to use a member property named
   * [Dynamic Forecast Multiplier] as shown in the following MDX query.
   */
   ...
   * functions specifically designed to navigate and aggregate information across time periods. For example,
   * year-to-date (YTD) totals are directly supported through the YTD function in MDX. In combination with the MDX
   * ParallelPeriod function, you can create calculated members to support direct comparison of totals across time
   * periods.
   *
   * <p>For example, the following table represents a comparison of YTD unit
   * sales between 1997 and 1998, run against the Sales cube in the FoodMart 2000 database.
   *
   * <p>The following MDX query uses three calculated members to illustrate
   * how to use the YTD and ParallelPeriod functions in combination to compare time periods.
   */
   ...
   * partially already by previous MDX; bug
   ...
   * empty SQL query generated when crossjoining more than two sets each containing just the 'all' member.
   */
  ...
  /**
   * Unit test for {@link StatisticsProvider} and implementations {@link JdbcStatisticsProvider} and {@link
   ...
   * MONDRIAN-1161</a>. It verifies that two queries can run at the same time.
   */
  ...
  /**
   * Dummy statistics provider for {@link mondrian.test.BasicQueryTest#testStatistics()}.
   */
   ...
   * It also makes sure that we do not leak a future to a segment which we are about to cancel.
   *
   * <p>It is disabled by default because it can make the JVM crash
   * pretty hard if it is run as part of the full test suite. Something to do with stack heap problems. Probably because
   * the test has to run on multiple threads at the same time.
   */
   ...
   * because the offset resolved to 0 and was used to fetch data directly out of the array.
   */
   ...
   * snowflake dimension causes exception
   * </a>
   */
