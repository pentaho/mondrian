...
public class DrillThroughTest extends FoodMartTestCase {
    ...
    public void testDrillThroughDupKeysAndMeasure() throws Exception {
        ...
        TestContext testContext = TestContext.instance().withSchema(
            ...
            + "    <Measure name=\"Frozen sqft M\" column=\"frozen_sqft\" aggregator=\"sum\"/>\n"
            + "    <Measure name=\"Grocery sqft M\" column=\"grocery_sqft\" aggregator=\"sum\"/>\n"
            + "    <Measure name=\"Meat sqft M\" column=\"meat_sqft\" aggregator=\"sum\"/>\n"
            + "    <Measure name=\"Store sqft M\" column=\"store_sqft\" aggregator=\"sum\"/>\n"
            + "  </Cube>\n"
            + "</Schema>\n"
        );
       ...
       final Result result = testContext.executeQuery(
           "WITH\n"
           + "SET [*NATIVE_CJ_SET] AS 'FILTER([Frozen sqft].[Frozen sqft].MEMBERS, NOT ISEMPTY ([Measures].[Store sqft M]))'\n"
           + "SET [*SORTED_ROW_AXIS] AS 'ORDER([*CJ_ROW_AXIS],[Frozen sqft].CURRENTMEMBER.ORDERKEY,BASC)'\n"
           + "SET [*BASE_MEMBERS__Measures_] AS '{[Measures].[Store sqft M]}'\n"
           ...
           + "SELECT\n"
           + "FILTER([*BASE_MEMBERS__Measures_],([Measures].CurrentMember Is [Measures].[Store sqft M])) ON COLUMNS\n"
       ...
       testContext.assertSqlEquals(
           "select store.frozen_sqft as Frozen sqft, store.grocery_sqft as Grocery sqft, store.meat_sqft as Meat sqft, store.store_sqft as Store sqft, store.store_sqft as Store sqft M_0 from foodmart.store as store where store.frozen_sqft = 2452 order by store.frozen_sqft ASC, store.grocery_sqft ASC, store.meat_sqft ASC, store.store_sqft ASC",
           sql,
           1);
   }
