...
public class FastBatchingCellReaderTest extends BatchTestCase {
  ...
  protected void setUp() throws Exception {
    ...
    final Statement statement = ( (RolapConnection) getTestContext().getConnection() ).getInternalStatement();
    ...
    Locus.push( locus );
    salesCube = (RolapCube) getTestContext().getConnection().getSchemaReader().withLocus().getCubes()[0];
  }
  ...
  public void testGroupBatchesForNonGroupableBatchesWithSorting() {
    ...
    BatchLoader.Batch maritalStatusBatch =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "marital_status", "M" ) );
  ...
  public void testGroupBatchesForNonGroupableBatchesWithConstraints() {
    ...
    BatchLoader.Batch genderBatch =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "gender", "F", constraint ) );
    BatchLoader.Batch maritalStatusBatch =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "marital_status", "M",
            constraint ) );
    ArrayList<BatchLoader.Batch> batchList = new ArrayList<BatchLoader.Batch>();
  ...
  public void testGroupBatchesForGroupableBatches() {
    ...
    BatchLoader.Batch superBatch =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, new String[0], new String[0],
            new String[0] ) ) {
  ...
  public void testGroupBatchesForGroupableBatchesAndNonGroupableBatches() {
    ...
    final BatchLoader.Batch group2Agg1 =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "education", "F" ) ) {
          boolean canBatch( BatchLoader.Batch batch ) {
            return false;
          }
        };
    BatchLoader.Batch group2Detailed =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "yearly_income", "" ) ) {
          boolean canBatch( BatchLoader.Batch batch ) {
  ...
  public void testGroupBatchesForTwoSetOfGroupableBatches() {
    ...
    BatchLoader.Batch batch1RollupOnGender =
        createBatch( fbcr, new String[] { tableTime, tableStore, tableProductClass }, new String[] { fieldYear,
    ...
    BatchLoader.Batch batch1RollupOnGenderAndProductDepartment =
        createBatch( fbcr, new String[] { tableTime, tableProductClass }, new String[] { fieldYear,
    ...
    BatchLoader.Batch batch1RollupOnStoreTypeAndProductDepartment =
        createBatch( fbcr, new String[] { tableTime, tableCustomer }, new String[] { fieldYear, fieldGender },
            new String[][] { fieldValuesYear, fieldValuesGender }, cubeNameSales, measureUnitSales );

    BatchLoader.Batch batch1Detailed =
        createBatch( fbcr, new String[] { tableTime, tableStore, tableProductClass, tableCustomer }, new String[] {
    ...
    BatchLoader.Batch batch2RollupOnStoreType =
        createBatch( fbcr, new String[] { tableWarehouse, tableTime, tableProductClass }, new String[] {
    ...
    BatchLoader.Batch batch2RollupOnProductFamilyAndWareHouseCountry =
        createBatch( fbcr, new String[] { tableTime, tableStore }, new String[] { fieldYear, fieldStoreType },
            new String[][] { fieldValuesYear, fieldValuesStoreType }, warehouseCube, measure2 );

    BatchLoader.Batch batch2Detailed =
        createBatch( fbcr, new String[] { tableWarehouse, tableTime, tableStore, tableProductClass }, new String[] {
  ...
  public void testAddToCompositeBatchForAggregationBatchAlreadyPartOfACompositeBatch() {
    ...
    BatchLoader.Batch aggBatchToAddToDetailedBatch =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "gender", "F" ) );
    BatchLoader.Batch aggBatchAlreadyInComposite =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "city", "F" ) );
    Map<AggregationKey, BatchLoader.CompositeBatch> batchGroups =
        new HashMap<AggregationKey, BatchLoader.CompositeBatch>();
  ...
  public void testAddToCompositeBatchForBothBatchAlreadyPartOfACompositeBatch() {
    ...
    BatchLoader.Batch aggBatchAlreadyInCompositeOfDetail =
        fbcr.new Batch( createRequest( cubeNameSales, measureUnitSales, "customer", "state_province", "F" ) );
  ...
  public void testCanBatchForSuperSet() {
    ...
    BatchLoader.Batch aggregationBatch =
        createBatch( fbcr, new String[] { tableTime, tableProductClass, tableProductClass }, new String[] { fieldYear,
    ...
    BatchLoader.Batch detailedBatch =
        createBatch( fbcr, new String[] { tableTime, tableProductClass, tableProductClass, tableCustomer },
  ...
  public void testCanBatchSummaryBatchWithDetailedBatchWithDistinctCount() {
    ...
    BatchLoader.Batch aggregationBatch =
        createBatch( fbcr, new String[] { tableTime }, new String[] { fieldYear }, new String[][] { fieldValuesYear },
  ...
  public void testNonSuperSet() {
    ...
    BatchLoader.Batch detailedBatch =
        createBatch( fbcr, new String[] { tableProductClass, tableProductClass, tableCustomer }, new String[] {
  ...
  public void testCanBatchForBatchesFromSameAggregationButDifferentRollupOption() {
    ...
    BatchLoader.Batch batch2 =
        createBatch( fbcr, new String[] { tableTime, tableTime, tableTime }, new String[] { fieldYear, "quarter",
  ...
  public void testCompositeBatchLoadAggregation() throws Exception {
    ...
    MondrianServer.forConnection( getTestContext().getConnection() ).getAggregationManager().cacheMgr.execute(
  ...
  public void testAggregateDistinctCount2ndParameter() {
    ...
    assertQueryReturns( "with\n" + "  set periods as [Time].[1997].[Q1].[1] : [Time].[1997].[Q4].[10]\n"
        ...
        + "  [Time].[agg]  ON COLUMNS,\n" + "  [Gender].[M] on ROWS\n" + "FROM [Sales]", "Axis #0:\n" + "{}\n"
            + "Axis #1:\n" + "{[Time].[agg]}\n" + "Axis #2:\n" + "{[Gender].[M]}\n" + "Row #0: 2,651\n" );
    assertQueryReturns( "WITH MEMBER [Measures].[My Distinct Count] AS \n"
  ...
  public void testCountDistinctAggWithOtherCountDistinctInContext() {
    ...
    TestContext testContext =
        TestContext.instance().create( null, "<Cube name=\"2CountDistincts\" defaultMeasure=\"Store Count\">\n"
    ...
    testContext.assertQueryReturns( "with member Store.agg as "
  ...
  public static class MyDelegatingInvocationHandler extends DelegatingInvocationHandler {
    ...
    /**
     * Handler for {@link mondrian.spi.Dialect#supportsGroupingSets()}.
     *
  ...
  public void testInMemoryAggSum() throws Exception {
    ...
    // Test with double
    Assert.assertEquals( 3.5, RolapAggregator.Sum.aggregate( Arrays.asList( dblSet1 ), Dialect.Datatype.Numeric ) );
    Assert.assertEquals( null, RolapAggregator.Sum.aggregate( Arrays.asList( dblSet2 ), Dialect.Datatype.Numeric ) );
    try {
    ...
    Assert.assertEquals( 4.6, RolapAggregator.Sum.aggregate( Arrays.asList( dblSet4 ), Dialect.Datatype.Numeric ) );

    // test with int
    Assert.assertEquals( 5, RolapAggregator.Sum.aggregate( Arrays.asList( intSet1 ), Dialect.Datatype.Integer ) );
    Assert.assertEquals( null, RolapAggregator.Sum.aggregate( Arrays.asList( intSet2 ), Dialect.Datatype.Integer ) );
    try {
  ...
  public void testInMemoryAggMin() throws Exception {
    ...
    // Test with double
    Assert.assertEquals( 0.0, RolapAggregator.Min.aggregate( Arrays.asList( dblSet1 ), Dialect.Datatype.Numeric ) );
    Assert.assertEquals( null, RolapAggregator.Min.aggregate( Arrays.asList( dblSet2 ), Dialect.Datatype.Numeric ) );
    try {
    ...
    Assert.assertEquals( 1.9, RolapAggregator.Min.aggregate( Arrays.asList( dblSet4 ), Dialect.Datatype.Numeric ) );

    // test with int
    Assert.assertEquals( 0, RolapAggregator.Min.aggregate( Arrays.asList( intSet1 ), Dialect.Datatype.Integer ) );
    Assert.assertEquals( null, RolapAggregator.Min.aggregate( Arrays.asList( intSet2 ), Dialect.Datatype.Integer ) );
    try {
  ...
  public void testInMemoryAggMax() throws Exception {
    ...
    // Test with double
    Assert.assertEquals( 2.4, RolapAggregator.Max.aggregate( Arrays.asList( dblSet1 ), Dialect.Datatype.Numeric ) );
    Assert.assertEquals( null, RolapAggregator.Max.aggregate( Arrays.asList( dblSet2 ), Dialect.Datatype.Numeric ) );
    Assert.assertEquals( -1.2, RolapAggregator.Max.aggregate( Arrays.asList( dblSet5 ), Dialect.Datatype.Numeric ) );
    try {
    ...
    Assert.assertEquals( 2.7, RolapAggregator.Max.aggregate( Arrays.asList( dblSet4 ), Dialect.Datatype.Numeric ) );

    // test with int
    Assert.assertEquals( 4, RolapAggregator.Max.aggregate( Arrays.asList( intSet1 ), Dialect.Datatype.Integer ) );
    Assert.assertEquals( null, RolapAggregator.Max.aggregate( Arrays.asList( intSet2 ), Dialect.Datatype.Integer ) );
    try {
    ...
    Assert.assertEquals( 7, RolapAggregator.Max.aggregate( Arrays.asList( intSet4 ), Dialect.Datatype.Integer ) );
  }
